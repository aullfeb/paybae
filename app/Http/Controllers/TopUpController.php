...
class TopUpController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (empty($secretKey) || str_contains($secretKey, 'YOUR_XENDIT_SECRET_KEY') || str_contains($secretKey, 'placeholder')) {
            // Local Mock fallback simulation if key is not configured
            $virtualAccountNumber = '88888' . str_pad(rand(0, 99999999), 8, '0', STR_PAD_LEFT);
        } else {
            try {
    ...
    public function simulate(Request $request)
    {
        ...
        $secretKey = config('xendit.secret_key');
        $isLocalVa = str_starts_with($validated['va_number'], '88888');

        if (empty($secretKey) || str_contains($secretKey, 'YOUR_XENDIT_SECRET_KEY') || str_contains($secretKey, 'placeholder') || $isLocalVa) {
            ...
            return response()->json(['success' => true, 'message' => 'Pembayaran berhasil disimulasikan secara lokal!']);
        }
